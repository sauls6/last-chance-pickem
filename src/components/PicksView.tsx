...
export const PicksView: React.FC<PicksViewProps> = ({
  ...
  // Load existing picks when user or week changes
  useEffect(() => {
    async function loadData() {
      if (!currentUser || matchups.length === 0) return;
      const gameIds = matchups.map((m) => m.id);
      const savedPicks = await getPicksForWeek(currentUser.userId, selectedWeek, gameIds);
      setPicks(savedPicks);

      const tb = await getTiebreaker(currentUser.userId, selectedWeek);
      if (tb !== null) {
        setTiebreakerState(tb.toString());
      } else {
        setTiebreakerState('');
      }

      const sp = await getGameSplits(selectedWeek, matchups);
      setSplits(sp);
    }
    loadData();
  }, [currentUser, selectedWeek, matchups]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setToastMessage('All picks saved successfully!');
      setTimeout(() => setToastMessage(null), 3000);
    } catch (e) {
      console.error(e);
      setToastMessage('Error saving picks');
      setTimeout(() => setToastMessage(null), 3000);
    } finally {
      setIsSaving(false);
    }
    ...
    <div className="w-full max-w-xl mx-auto px-4 pt-4 pb-28">
      ...
      <div className="mb-4 text-center">
        ...
        <p className="text-xs text-[#9AA0A6] max-w-md mx-auto">
          Predict the winner of all 6 fantasy matchups in our 12-team league.
        </p>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between p-3 rounded-xl bg-[#0E1013] border border-[#1C1F26] text-xs">
        ...
        {currentUser && (
          <span className="font-semibold text-[#6A85FA] bg-[#6A85FA]/10 px-2.5 py-1 rounded-full border border-[#6A85FA]/30">
            {pickedCount} / {totalMatchups} Selected
          </span>
        )}
      </div>
      ...
      {isWeekOpen && matchups.length > 0 && (
        <div className="sticky bottom-20 z-40 mt-6 pt-2">
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="w-full py-3.5 px-6 rounded-2xl bg-[#6A85FA] hover:bg-[#5872ea] active:scale-[0.99] text-white font-bold text-sm tracking-wide shadow-[0_0_24px_rgba(106,133,250,0.45)] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
          >
            <CheckCircle2 className="w-4 h-4 stroke-[2.5]" />
            {isSaving ? 'Saving Picks...' : `Save Picks (${pickedCount}/${totalMatchups})`}
          </button>
        </div>
      )}
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 px-4 py-2.5 rounded-full bg-[#1C2230] border border-[#6A85FA] text-xs font-semibold text-[#F2F2E8] shadow-[0_8px_24px_rgba(0,0,0,0.8)] flex items-center gap-2 animate-bounce">
          <Sparkles className="w-3.5 h-3.5 text-[#6A85FA]" />
          {toastMessage}
        </div>
      )}
    </div>
